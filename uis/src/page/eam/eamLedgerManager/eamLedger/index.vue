    ...
    <el-row :gutter="10" style="margin:0px 8px;">
        ...
        <el-card :style="{height:(this.$store.state.contentHeight-25)+'px'}">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="按系统" name="BY_SYSTEM">
              <el-tree :data="treeData" :props="defaultProps" @node-click="handleSecondNodeClick">
                <span class="custom-tree-node" slot-scope="{ node, data}">
                  <i :class="data.defect==='MAJOR'?'major-color':(data.defect==='NONE'?'other-color':'normal-color')"></i>
                  <span style="margin-left:5px;">{{ node.label }}</span>
                </span></el-tree>
          ...
          <div class="table-search-wrapper">
            <el-input placeholder="根据设备名称查询" size="small" v-model="queryParam.name">
              <template slot="append">
                <el-button type="primary" :size="GlobalCss.buttonSize" icon="el-icon-search" @click="initTable()">
                </el-button>
              </template>
            </el-input>
          </div>
          <div class="queryBodys">
            <el-form ref="ruleForm" label-suffix="：" label-position="left" style="width:700px;" size="mini"
              label-width="100px" :inline-message="true" :status-icon="true" class="demo-ruleForm">
              <el-form-item label="投运时长">
                <el-radio-group v-model="queryParam.time" @change="initTable()">
                  <el-radio border label="ALL">全部</el-radio>
                  <el-radio border label="Y">&lt; 1年</el-radio>
                  <el-radio border label="LTY">&lt; 3年</el-radio>
                  <el-radio border label="GTY">&gt;= 3年</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="设备状态">
                <el-radio-group v-model="queryParam.status" @change="initTable()">
                  <el-radio border label="ALL">全部</el-radio>
                  <el-radio border label="正常">正常</el-radio>
                  <el-radio border label="已报废">已报废</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="资料完整度">
                <el-radio-group v-model="queryParam.complete" @change="initTable()">
                  <el-radio border label="ALL">全部</el-radio>
                  <el-radio border label="LTHALF">&lt;50%</el-radio>
                  <el-radio border label="GTHALF">&gt;50%</el-radio>
                </el-radio-group>
              </el-form-item>
            </el-form>
          </div>
      ...
      <el-button type="success"
          ...
          <div class="table-list">
            <el-table :data="tableData" style="width: 100%" border :height="tableHeight" highlight-current-row
              :size="GlobalCss.buttonSize">
              ...
              <el-table-column prop="completePoint" label="资料完整度" align="center" sortable>
                <template slot-scope="scope">
                  <el-progress :percentage="scope.row.completePoint" :color="customColorMethod"></el-progress>
                </template>
              </el-table-column>
              <el-table-column prop="factoryName" label="生产厂家" align="center" sortable :show-overflow-tooltip="true">
              </el-table-column>
              <el-table-column prop="installLocationName" sortable label="安装位置" align="center"></el-table-column>
              <template v-if="activeName==='BY_SYSTEM'">
                <el-table-column prop="deviceSystem" label="设备系统" sortable align="center"></el-table-column>
              </template>
              <template v-if="activeName==='BY_PROFESSIONA'">
                <el-table-column prop="profession" label="设备专业" sortable align="center"></el-table-column>
              </template>
            ...
            <div class="tableFooter">
              <div class="pagination">
                <el-pagination background @current-change="changePage" :current-page="queryParam.page"
                  :page-size="queryParam.size" layout="total, prev, pager, next, jumper" :total="total">
                </el-pagination>
              </div>
            </div>
          </div>
