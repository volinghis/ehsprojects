export default {
  ...
  methods: {
    handleAdd: function () {
      this.$router.push({ name: 'partsBaseInfoEdit', flag: 'edit', replace: true })
    },
    handleDelete: function (row, index) {
      this.$confirm('确定删除该数据?', '温馨提示', {
        ...
        this.$axios.get(this.GlobalVars.globalServiceServlet + '/eam/partsBaseInfo/deletePartsAccount', { params: { key: row.key } }).then(res => {
    ...
    handleView: function (row) {
      this.$axios.get(this.GlobalVars.globalServiceServlet + '/eam/partsParams/getAllPartsParamByKey', { params: { key: row.key } }).then(res => {
        if (res.data.length > 0) {
          ...
          this.$router.push({ name: 'partsBaseInfoDetails', params: { data: row, pData: this.paramsDatas, flag: 'view', replace: true } })
        }
      })
    },
    handleEdit: function (row) {
      this.$axios.get(this.GlobalVars.globalServiceServlet + '/eam/partsParams/getAllPartsParamByKey', { params: { key: row.key } }).then(res => {
        if (res.data.length > 0) {
          this.paramsDatas = res.data
          this.$router.push({ name: 'partsBaseInfoEdit', params: { data: row, pData: this.paramsDatas, flag: 'edit', replace: true } })
        }
      })
    },
    ...
    getTableData: function () {
      this.$axios.get(this.GlobalVars.globalServiceServlet + '/eam/partsBaseInfo/getPartsBaseInfos', this.form).then(res => {
