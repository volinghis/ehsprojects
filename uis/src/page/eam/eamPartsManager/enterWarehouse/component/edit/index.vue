<template>
  <div :style="{height:$store.state.contentHeight+'px',padding:'0px 20px',background:'#fff'}" class="divHeight">
    <el-form ref="form"  :model="form" :rules="rules" label-suffix="：" label-position="right" label-width="100px" :size="GlobalCss.buttonSize" class="formWidth">
      ...
      <el-button type="primary" icon="el-icon-plus" class="buttonHeight" :size="GlobalCss.buttonSize" @click="handlerSelect" v-show="showButton">选择商品</el-button>
      <el-button type="primary" icon="el-icon-plus" class="buttonHeight" :size="GlobalCss.buttonSize" @click="handlerAdd" v-show="showButton">添加商品</el-button>
      <!--商品表格-->
    ...
    <el-dialog title="选择商品" :visible.sync="dialogVisible" width="70%" :before-close="handleClose" destroy-on-close>
