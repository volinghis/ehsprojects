<template>
  <div class="siders">
    <el-menu class="el-menu-vertical-demo" :default-active="$router.currentRoute.name"  :background-color="GlobalCss.menuBg" :text-color="GlobalCss.menuText"
      :active-text-color="GlobalCss.menuActiveText" @select="menuSelect" :unique-opened="true">
      <template v-for="item in $store.state.menuDatas">
        <el-menu-item :index="item.key" :key="item.key" v-if="item.leaf&&!item.business">
          <i :class="'fa '+(item.icon?item.icon:'')+'  ' "></i> {{ item.label }}
        </el-menu-item>
        <template v-else>
