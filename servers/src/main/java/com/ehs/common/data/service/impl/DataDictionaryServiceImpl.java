...
public List<DataDictionary> findDataDictByParentKey(String key) {
	...
	*/
	...
	public List<DataDictionary> getFirstNode(String parentCode) {
		...
		/**
		*
		* @see com.ehs.common.data.service.DataDictionaryService#getAllDatasTable(java.lang.String,
		com.ehs.common.organization.bean.OrgQueryBean)
		* @Function: DataDictionaryServiceImpl.java
		* @Description: 该函数的功能描述
		* @version: v1.0.0
		* @author: zhaol
		...
		*/
		...
		Page<DataDictionary> datas=dataDictionaryDao.findDatasByParentCode(parentCode,queryBean.getQuery(), pageRequest);
			...
			Page<DataDictionary> datas=dataDictionaryDao.findAllDatas(queryBean.getQuery(), pageRequest);
				List<DataDictionary> dataDicts = datas.stream().filter(s -> !StringUtils.equals(s.getParentKey(),
					"dataDict")).collect(Collectors.toList());
					if (dataDicts!=null) {
					...
					*
					* @see
					com.ehs.common.data.service.DataDictionaryService#saveDataDictionary(com.ehs.common.data.entity.DataDictionary)
					* @Function: DataDictionaryServiceImpl.java
					* @Description: 该函数的功能描述
					*
					* @version: v1.0.0
